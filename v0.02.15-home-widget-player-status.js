...
function v0215HeroWeatherCardHTML(){
  ...
  return `<div class="hero-weather-card">
    <div class="hero-weather-hero-row">
      <img class="hero-weather-icon" src="${asset(w.icon)}" alt="${esc(w.condition)}">
    ...
    <div class="hero-weather-mobile-temp-row weather-forecast-trigger" id="weatherForecastTrigger" role="button" tabindex="0" aria-label="Open 5-day weather forecast" title="Open 5-day forecast">
...
function heroHudStatRowHTML(key,label,data){
  ...
  const gearNote=data.gear?`${label} ${Math.round(Number(data.value||0))}: ${Math.round(Number(data.base||0))} base plus ${Math.round(Number(data.gear||0))} from equipped gear`:'';
  return `<div class="hero-hud-stat-row hero-hud-stat-row-plain" data-v8-stat="${key}"${gearNote?` role="group" aria-label="${esc(gearNote)}" title="${esc(gearNote)}"`:''}>
    ${icon}
    <span class="hero-hud-stat-label">${esc(label)}</span>
  ...
  return `<section class="player-status-panel widget-player-status home-hero-scene home-hero-experiment-b" data-player-variant="widget">
    <div class="hero-hud${showWeather?'':' no-weather'}">
      ...
      <div class="hero-hud-side">
        ...
        <div class="hero-hud-gold hero-info-row${gold<0?' is-deficit':''}"><img class="hero-info-icon" src="${HERO_GOLD_SYMBOL}" alt=""><span class="hero-info-value hero-info-value-gold"${gold<0?' title="Purchases paused until your balance is above 0" aria-label="Gold, negative balance. Purchases paused until your balance is above 0"':''}>${gold.toLocaleString()}</span>${typeof economyHudRewardsHTML==='function'?economyHudRewardsHTML():''}</div>
      </div>
    </div>
  </section>`;
}
