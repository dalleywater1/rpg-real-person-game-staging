...
function exerciseResultRowHTML(ex,t){
  ...
  return `<div class="exercise-result-row">
    ...
    <button type="button" class="exercise-fav-btn ${fav?'active':''}" data-el-fav="${ex.id}" aria-label="Favourite ${esc(ex.name)}" aria-pressed="${fav?'true':'false'}"><img src="${asset('Training/UI/UI_STAR.svg')}" alt=""></button>
...
function wbExerciseResultsHTML(){
  ...
  const results=muscleFilter?searchExercises('',{muscle:muscleFilter}):[];/* the whole zone, scrolling (Jay 2026-09-25); it used to stop at 8 */
  return `<div class="mg-results">
    ${muscleSelectorSelectedZone?(results.length?`<div class="exercise-library-results" role="group" aria-label="${esc(zone?zone.label:muscleTagLabel(muscleSelectorSelectedZone))} exercises, ${results.length} listed">${results.map(ex=>`<div class="exercise-result-row"><div><strong>${esc(ex.name)}</strong><span>${[ex.primaryMuscle,ex.equipment,ex.movement].filter(Boolean).map(esc).join(' · ')}</span></div><button type="button" class="exercise-add-btn" data-wb-pick="${ex.id}" aria-label="Add ${esc(ex.name)}"><img src="${asset('Training/UI/UI_ADD.svg')}" alt=""></button></div>`).join('')}</div>`:'<p class="helper">No exercises tagged for this zone yet.</p>'):''}
...
function wbAddExercise(ex){
  ...
  renderTrainingAreaKeepLists();/* keep the zone list where it was: adding must not throw you back to row 1 */
  toast(`Added ${ex.name}`);/* the live region tells a screen-reader user it worked */
}
...
function duplicateWorkoutTemplate(templateId){
  ...
  /* the copy lands at the far end of the row: bring it into view so you can see it worked */
  const nc=document.querySelector(`[data-my-workout="${copy.id}"]`);if(nc&&nc.scrollIntoView)nc.scrollIntoView({block:'nearest',inline:'nearest'});
}
